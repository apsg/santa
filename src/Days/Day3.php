...
//    const PATH2 = 'U62,R66,U55,R34,D71,R55,D58,R83';
    ...
    {
        $path1 = new Path(self::PATH1);
        $path2 = new Path(self::PATH2);

        print_r($path1->intersections($path2));

        echo min(array_map('self::distance', $path2->intersections($path1)));
    }
    ...
    public static function part2()
    {
        ...
        $fn = function ($item) use ($path1, $path2) {
            return $path1->distanceTo($item) + $path2->distanceTo($item);
        };

        static::print($path2->count());

        echo min(array_map($fn, $intersections));
    }

    public static function distance(Point $point1, Point $point2 = null) : int
    {
        if ($point2 === null) {
